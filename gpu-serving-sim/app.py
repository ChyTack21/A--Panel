...
REQUESTS_TOTAL = Counter("gpu_serving_requests_total", "Total serving lane requests", ["instance", "status", "source", "profile", "endpoint"])
REQUEST_DURATION = Histogram(
    "gpu_serving_request_seconds",
    "Serving lane end-to-end request latency",
    ["instance", "source", "profile", "endpoint"],
...
REQUEST_MATRIX = Gauge("gpu_serving_request_matrix_size", "Matrix size requested by the latest serving call", ["instance", "source", "profile"])
REQUEST_STEPS = Gauge("gpu_serving_request_steps", "Step count requested by the latest serving call", ["instance", "source", "profile"])
REQUEST_BATCHES = Gauge("gpu_serving_request_batches", "Batch count requested by the latest serving call", ["instance", "source", "profile"])
REQUEST_COST = Gauge("gpu_serving_request_cost_index", "Approximate serving cost index matrix_size*steps*batches", ["instance", "source", "profile"])
SOURCE_GAUGE = Gauge("gpu_serving_request_source_active", "Latest observed serving request source/profile", ["instance", "source", "profile"])
...
def request_labels(request: Request) -> dict[str, str]:
    source = (request.headers.get("X-Sim-Source") or "manual").strip() or "manual"
    profile = (request.headers.get("X-Sim-Profile") or "manual").strip() or "manual"
    return {"source": source[:40], "profile": profile[:40], "endpoint": "/infer"}


def set_source_profile(source: str, profile: str) -> None:
    known = [
        ("manual", "manual"),
        ("gpu-loadgen", "idle"),
        ("gpu-loadgen", "light"),
    ...
    for known_source, known_profile in known:
        SOURCE_GAUGE.labels(instance=INSTANCE, source=known_source, profile=known_profile).set(
            1 if known_source == source and known_profile == profile else 0
        )
    if (source, profile) not in known:
        SOURCE_GAUGE.labels(instance=INSTANCE, source=source, profile=profile).set(1)


def infer_on_gpu(payload: InferenceRequest, labels_meta: dict[str, str]) -> dict[str, object]:
    ...
    metric_labels = {"instance": INSTANCE, **labels_meta}
    ...
    try:
        ...
        TOTAL_REQUESTS += 1
        set_source_profile(labels_meta["source"], labels_meta["profile"])
        device_index = 0
        ...
        REQUESTS_TOTAL.labels(status="success", **metric_labels).inc()
        REQUEST_DURATION.labels(**metric_labels).observe(duration)
        ...
        REQUEST_MATRIX.labels(instance=INSTANCE, source=labels_meta["source"], profile=labels_meta["profile"]).set(matrix_size)
        REQUEST_STEPS.labels(instance=INSTANCE, source=labels_meta["source"], profile=labels_meta["profile"]).set(steps)
        REQUEST_BATCHES.labels(instance=INSTANCE, source=labels_meta["source"], profile=labels_meta["profile"]).set(batches)
        REQUEST_COST.labels(instance=INSTANCE, source=labels_meta["source"], profile=labels_meta["profile"]).set(matrix_size * steps * batches)
        LAST_ERROR = ""
        del accumulator
        return {
            "ok": True,
            "gpu": labels["gpu"],
            "uuid": labels["uuid"],
            "source": labels_meta["source"],
            "profile": labels_meta["profile"],
            "endpoint": labels_meta["endpoint"],
        ...
        LAST_ERROR = str(exc)
        REQUESTS_TOTAL.labels(status="error", **metric_labels).inc()
...
def startup() -> None:
    ACTIVE_GAUGE.labels(instance=INSTANCE).set(0)
    QUEUE_GAUGE.labels(instance=INSTANCE).set(0)
    CUDA_GAUGE.labels(instance=INSTANCE).set(1 if torch.cuda.is_available() else 0)
    set_source_profile("manual", "manual")
    REQUEST_MATRIX.labels(instance=INSTANCE, source="manual", profile="manual").set(DEFAULT_MATRIX_SIZE)
    REQUEST_STEPS.labels(instance=INSTANCE, source="manual", profile="manual").set(DEFAULT_STEPS)
    REQUEST_BATCHES.labels(instance=INSTANCE, source="manual", profile="manual").set(DEFAULT_BATCHES)
    REQUEST_COST.labels(instance=INSTANCE, source="manual", profile="manual").set(DEFAULT_MATRIX_SIZE * DEFAULT_STEPS * DEFAULT_BATCHES)
...
@app.post("/infer")
def infer(payload: InferenceRequest, request: Request) -> dict[str, object]:
    return infer_on_gpu(payload, request_labels(request))
